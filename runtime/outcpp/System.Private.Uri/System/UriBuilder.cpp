...
void UriBuilder___::set_Fragment(String value) {
  ...
  if (value->get_Length() > 0 && value[0] != 35) {
    value = "#" + value;
  }
  _fragment = value;
  _changed = true;
...
void UriBuilder___::set_Host(String value) {
  ...
  _host = value;
  if (_host->Contains(58) && _host[0] != 91) {
    _host = "[" + _host + "]";
  }
  _changed = true;
}
...
void UriBuilder___::set_Path(String value) {
  _path = (String::in::IsNullOrEmpty(value) ? "/" : Uri::in::InternalEscapeString(value->Replace(92, 47)));
  _changed = true;
}
...
void UriBuilder___::set_Query(String value) {
  ...
  if (value->get_Length() > 0 && value[0] != 63) {
    value = "?" + value;
  }
  _query = value;
  _changed = true;
...
String UriBuilder___::ToString() {
  ...
  return text + _username + ((_password->get_Length() > 0) ? (":" + _password) : String::in::Empty) + ((_username->get_Length() > 0) ? "@" : String::in::Empty) + _host + ((_port != -1 && _host->get_Length() > 0) ? (":" + _port) : String::in::Empty) + ((_host->get_Length() > 0 && _path->get_Length() != 0 && _path[0] != 47) ? "/" : String::in::Empty) + _path + _query + _fragment;
}
