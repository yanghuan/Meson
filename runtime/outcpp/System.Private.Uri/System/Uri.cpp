...
Array<String> Uri___::get_Segments() {
  ...
  while (num < privateAbsolutePath->get_Length()) {
    Int32 num2 = privateAbsolutePath->IndexOf(u'/', num);
    if (num2 == -1) {
    ...
    switch (get_HostType()) {
      ...
      case Flags::BasicHostType:
        if (InFact(Flags::HostNotCanonical | Flags::E_HostNotCanonical)) {
          ...
          UriHelper::UnescapeString(text, 0, text->get_Length(), dest, u'￿', u'￿', u'￿', UnescapeMode::Unescape | UnescapeMode::UnescapeAll, _syntax, false);
          text = dest.ToString();
        }
        break;
    }
...
void Uri___::GetCombinedString(Uri baseUri, String relativeStr, Boolean dontEscape, String& result) {
  for (Int32 i = 0; i < relativeStr->get_Length() && relativeStr[i] != u'/' && relativeStr[i] != u'\\' && relativeStr[i] != u'?' && relativeStr[i] != u'#'; i++) {
    if (relativeStr[i] == u':') {
      if (i < 2) {
        break;
      }
...
String Uri___::GetLocalPath() {
  ...
  if (get_IsUncOrDosPath()) {
    ...
    if (NotAny(Flags::HostNotCanonical | Flags::PathNotCanonical | Flags::ShouldBeCompressed)) {
      ...
      String text = ((get_IsImplicitFile() && _info->Offset.Host == ((!get_IsDosPath()) ? 2 : 0) && _info->Offset.Query == _info->Offset.End) ? _string : ((get_IsDosPath() && (_string[num] == u'/' || _string[num] == u'\\')) ? _string->Substring(num + 1, _info->Offset.Query - num - 1) : _string->Substring(num, _info->Offset.Query - num)));
      if (get_IsDosPath() && text[1] == u'|') {
    ...
    if (get_IsUncPath()) {
      ...
      array[1] = u'\\';
      destPosition = 2;
      UriHelper::UnescapeString(host, 0, host->get_Length(), array, destPosition, u'￿', u'￿', u'￿', UnescapeMode::CopyOnly, _syntax, false);
    } else if (_string[num] == u'/' || _string[num] == u'\\') {
      num++;
    }
    ...
    UriHelper::UnescapeString(_string, num, _info->Offset.Query, array, destPosition, u'￿', u'￿', u'￿', unescapeMode, _syntax, true);
    if (array[1] == u'|') {
      array[1] = u':';
    }
...
Char Uri___::HexUnescape(String pattern, Int32& index) {
  ...
  if (pattern[index] == u'%' && pattern->get_Length() - index >= 3) {
...
Boolean Uri___::IsHexEncoding(String pattern, Int32 index) {
  if (pattern->get_Length() - index >= 3 && pattern[index] == u'%' && IsHexDigit(pattern[index + 1])) {
...
Boolean Uri___::CheckSchemeName(String schemeName) {
  ...
  for (Int32 num = schemeName->get_Length() - 1; num > 0; num--) {
    if (!UriHelper::IsAsciiLetterOrDigit(schemeName[num]) && schemeName[num] != u'+' && schemeName[num] != u'-' && schemeName[num] != u'.') {
      return false;
    }
  }
  return true;
...
Boolean Uri___::Equals(Object comparand) {
  ...
  if (!get_UserDrivenParsing() && !result->get_UserDrivenParsing() && get_Syntax()->get_IsSimple() && result->get_Syntax()->get_IsSimple()) {
    if (InFact(Flags::CanonicalDnsHost) && result->InFact(Flags::CanonicalDnsHost)) {
      ...
      if (num < _info->Offset.Path && _string[num] != u':') {
        return false;
      }
      if (num3 < path && string[num3] != u':') {
        return false;
      }
    } else {
      EnsureHostString(false);
...
String Uri___::InternalEscapeString(String rawString) {
  if (rawString != nullptr) {
    return UriHelper::EscapeString(rawString, true, UriHelper::get_UnreservedReservedTable(), u'?', u'#');
  }
  return String::in::Empty;
}
  ...
  _flags &= ~(Flags::IndexMask | Flags::UserDrivenParsing);
  {
    ...
    if (_syntax->IsAllSet(UriSyntaxFlags::AllowEmptyHost | UriSyntaxFlags::AllowDOSPath) && NotAny(Flags::ImplicitFile) && num + 1 < num2) {
      ...
      if (_syntax->InFact(UriSyntaxFlags::FileLikeUri) || i - num <= 3) {
        ...
        Char c;
        if (i + 1 < num2 && ((c = *(ptr2 + i + 1)) == u':' || c == u'|') && UriHelper::IsAsciiLetter(*(ptr2 + i))) {
          if (i + 2 >= num2 || ((c = *(ptr2 + i + 2)) != u'\\' && c != u'/')) {
        ...
        } else if (_syntax->InFact(UriSyntaxFlags::FileLikeUri) && i - num >= 2 && i - num != 3 && i < num2 && *(ptr2 + i) != u'?' && *(ptr2 + i) != u'#') {
    ...
    if ((_flags & (Flags::DosPath | Flags::UncPath | Flags::UnixPath)) == Flags::Zero) {
      if (num + 2 <= num2) {
        ...
        Char c3 = *(ptr2 + num + 1);
        if (_syntax->InFact(UriSyntaxFlags::MustHaveAuthority)) {
          if ((c2 != u'/' && c2 != u'\\') || (c3 != u'/' && c3 != u'\\')) {
            return ParsingError::BadAuthority;
          }
          _flags |= Flags::AuthorityFound;
          num += 2;
        } else if (_syntax->InFact(UriSyntaxFlags::OptionalAuthority) && (InFact(Flags::AuthorityFound) || (c2 == u'/' && c3 == u'/'))) {
          _flags |= Flags::AuthorityFound;
          num += 2;
    ...
    if (num < num2) {
      Char c4 = *(ptr2 + num);
      if (c4 == u'\\' && NotAny(Flags::ImplicitFile) && _syntax->NotAny(UriSyntaxFlags::AllowDOSPath)) {
  ...
  if (!get_UserDrivenParsing()) {
    ...
    if ((cF & Flags::ImplicitFile) != Flags::Zero) {
      ...
      if (StaticInFact(cF, Flags::UncPath)) {
        i += 2;
        for (Int32 num = (Int32)(cF & Flags::IndexMask); i < num && (_string[i] == u'/' || _string[i] == u'\\'); i++) {
        }
      }
    } else {
      ...
      if ((cF & Flags::AuthorityFound) != Flags::Zero) {
        if (_string[i] == u'\\' || _string[i + 1] == u'\\') {
          flag = true;
        }
        i += 2;
        if ((cF & (Flags::DosPath | Flags::UncPath)) != Flags::Zero) {
          for (Int32 num2 = (Int32)(cF & Flags::IndexMask); i < num2 && (_string[i] == u'/' || _string[i] == u'\\'); i++) {
            flag = true;
          }
        }
      }
  ...
  if (text->get_Length() != 0) {
    if (get_HostType() == Flags::BasicHostType) {
      ...
      Check check;
      {
        Char* ptr = text;
        Char* str = ptr;
        check = CheckCanonical(str, idx, text->get_Length(), u'￿');
      }
      ...
      if ((check & (Check::EscapedCanonical | Check::BackslashInPath)) != Check::EscapedCanonical) {
        ...
        if (NotAny(Flags::UserEscaped)) {
          text = UriHelper::EscapeString(text, !get_IsImplicitFile(), UriHelper::get_UnreservedReservedTable(), u'?', u'#');
        }
      }
...
String Uri___::GetEscapedParts(UriComponents uriParts) {
  ...
  if ((uriParts & UriComponents::Path) != 0) {
    if (InFact(Flags::ShouldBeCompressed | Flags::FirstSlashAbsent | Flags::BackslashInPath)) {
      num = (UInt16)(num | 16u);
    } else if (get_IsDosPath() && _string[_info->Offset.Path + get_SecuredPathIndex() - 1] == u'|') {
      num = (UInt16)(num | 16u);
    }
...
String Uri___::ReCreateParts(UriComponents parts, UInt16 nonCanonical, UriFormat formatAs) {
    ...
    if ((nonCanonical & 2u) != 0) {
      switch (formatAs) {
        case UriFormat::UriEscaped:
          if (NotAny(Flags::UserEscaped)) {
            array = UriHelper::EscapeString(MemoryExtensions::AsSpan(_string, _info->Offset.User, _info->Offset.Host - _info->Offset.User), array, num, true, u'?', u'#');
            break;
          }
        ...
        case UriFormat::SafeUnescaped:
          array = UriHelper::UnescapeString(_string, _info->Offset.User, _info->Offset.Host - 1, array, num, u'@', u'/', u'\\', InFact(Flags::UserEscaped) ? UnescapeMode::Unescape : UnescapeMode::EscapeUnescape, _syntax, false);
          array[num++] = u'@';
          break;
        case UriFormat::Unescaped:
          array = UriHelper::UnescapeString(_string, _info->Offset.User, _info->Offset.Host, array, num, u'￿', u'￿', u'￿', UnescapeMode::Unescape | UnescapeMode::UnescapeAll, _syntax, false);
          break;
        default:
          array = UriHelper::UnescapeString(_string, _info->Offset.User, _info->Offset.Host, array, num, u'￿', u'￿', u'￿', UnescapeMode::CopyOnly, _syntax, false);
          break;
      }
    } else {
      UriHelper::UnescapeString(_string, _info->Offset.User, _info->Offset.Host, array, num, u'￿', u'￿', u'￿', UnescapeMode::CopyOnly, _syntax, false);
    }
  ...
  if ((parts & UriComponents::Host) != 0 && text->get_Length() != 0) {
    ...
    array = UriHelper::UnescapeString(text, 0, text->get_Length(), array, num, u'/', u'?', u'#', unescapeMode, _syntax, false);
  ...
  if ((parts & UriComponents::Path) != 0) {
    ...
    if (parts == UriComponents::Path) {
      Int32 startIndex;
      if (InFact(Flags::AuthorityFound) && num != 0 && array[0] == u'/') {
        startIndex = 1;
        num--;
      } else {
        startIndex = 0;
  ...
  if ((parts & UriComponents::Query) != 0 && _info->Offset.Query < _info->Offset.Fragment) {
    ...
    if ((nonCanonical & 32u) != 0) {
      switch (formatAs) {
        case UriFormat::UriEscaped:
          if (NotAny(Flags::UserEscaped)) {
            array = UriHelper::EscapeString(MemoryExtensions::AsSpan(_string, startIndex, _info->Offset.Fragment - startIndex), array, num, true, u'#');
          } else {
            UriHelper::UnescapeString(_string, startIndex, _info->Offset.Fragment, array, num, u'￿', u'￿', u'￿', UnescapeMode::CopyOnly, _syntax, true);
          }
          break;
        case (UriFormat)32767:
          array = UriHelper::UnescapeString(_string, startIndex, _info->Offset.Fragment, array, num, u'#', u'￿', u'￿', (InFact(Flags::UserEscaped) ? UnescapeMode::Unescape : UnescapeMode::EscapeUnescape) | UnescapeMode::V1ToStringFlag, _syntax, true);
          break;
        case UriFormat::Unescaped:
          array = UriHelper::UnescapeString(_string, startIndex, _info->Offset.Fragment, array, num, u'#', u'￿', u'￿', UnescapeMode::Unescape | UnescapeMode::UnescapeAll, _syntax, true);
          break;
        default:
          array = UriHelper::UnescapeString(_string, startIndex, _info->Offset.Fragment, array, num, u'#', u'￿', u'￿', InFact(Flags::UserEscaped) ? UnescapeMode::Unescape : UnescapeMode::EscapeUnescape, _syntax, true);
          break;
      }
    } else {
      UriHelper::UnescapeString(_string, startIndex, _info->Offset.Fragment, array, num, u'￿', u'￿', u'￿', UnescapeMode::CopyOnly, _syntax, true);
      ...
      switch (formatAs) {
        case UriFormat::UriEscaped:
          if (NotAny(Flags::UserEscaped)) {
            ...
            UriHelper::UnescapeString(_string, startIndex, _info->Offset.End, array, num, u'￿', u'￿', u'￿', UnescapeMode::CopyOnly, _syntax, false);
          }
          break;
        case (UriFormat)32767:
          array = UriHelper::UnescapeString(_string, startIndex, _info->Offset.End, array, num, u'#', u'￿', u'￿', (InFact(Flags::UserEscaped) ? UnescapeMode::Unescape : UnescapeMode::EscapeUnescape) | UnescapeMode::V1ToStringFlag, _syntax, false);
          break;
        case UriFormat::Unescaped:
          array = UriHelper::UnescapeString(_string, startIndex, _info->Offset.End, array, num, u'#', u'￿', u'￿', UnescapeMode::Unescape | UnescapeMode::UnescapeAll, _syntax, false);
          break;
        default:
          array = UriHelper::UnescapeString(_string, startIndex, _info->Offset.End, array, num, u'#', u'￿', u'￿', InFact(Flags::UserEscaped) ? UnescapeMode::Unescape : UnescapeMode::EscapeUnescape, _syntax, false);
          break;
      }
    } else {
      UriHelper::UnescapeString(_string, startIndex, _info->Offset.End, array, num, u'￿', u'￿', u'￿', UnescapeMode::CopyOnly, _syntax, false);
  ...
  if (!get_UserDrivenParsing()) {
    ...
    UriSyntaxFlags flags2 = _syntax->get_Flags();
    {
      ...
      if (get_IsImplicitFile()) {
        ...
        if ((_flags & Flags::AuthorityFound) != Flags::Zero && (scheme + i + 3 >= length || *(ptr2 + scheme + i + 1) != u'/' || *(ptr2 + scheme + i + 2) != u'/')) {
          flags |= Flags::SchemeNotCanonical;
        }
      }
      if ((_flags & Flags::HasUserInfo) != Flags::Zero) {
        scheme = _info->Offset.User;
        check = CheckCanonical(ptr2, scheme, _info->Offset.Host, u'@');
    ...
    if (flag) {
      ...
      if (get_IsImplicitFile() || (flags2 & (UriSyntaxFlags::MayHaveQuery | UriSyntaxFlags::MayHaveFragment)) == 0) {
        ...
        Int32 num2 = ((!_syntax->InFact(UriSyntaxFlags::MayHaveQuery)) ? MemoryExtensions::IndexOf(span, u'#') : ((!_syntax->InFact(UriSyntaxFlags::MayHaveFragment)) ? MemoryExtensions::IndexOf(span, u'?') : MemoryExtensions::IndexOfAny(span, u'?', u'#')));
    ...
    }
    {
      Char* ptr3 = _string;
      Char* ptr4 = ptr3;
      check = ((!get_IsImplicitFile() && (flags2 & (UriSyntaxFlags::MayHaveQuery | UriSyntaxFlags::MayHaveFragment)) != 0) ? CheckCanonical(ptr4, scheme, length, ((flags2 & UriSyntaxFlags::MayHaveQuery) != 0) ? u'?' : (_syntax->InFact(UriSyntaxFlags::MayHaveFragment) ? u'#' : u'￾')) : CheckCanonical(ptr4, scheme, length, u'￿'));
    ...
    if (flag) {
      Int32 start2 = num;
      if (num < _originalUnicodeString->get_Length() && _originalUnicodeString[num] == u'?') {
        if ((flags2 & UriSyntaxFlags::MayHaveFragment) != 0) {
          num++;
          Int32 num3 = MemoryExtensions::IndexOf(MemoryExtensions::AsSpan(_originalUnicodeString, num), u'#');
    ...
    _info->Offset.Query = (UInt16)scheme;
    {
      ...
      if (scheme < length && *(ptr6 + scheme) == u'?') {
        scheme++;
        check = CheckCanonical(ptr6, scheme, length, ((flags2 & UriSyntaxFlags::MayHaveFragment) != 0) ? u'#' : u'￾');
        if ((check & Check::DisplayCanonical) == 0) {
    ...
    _info->Offset.Fragment = (UInt16)scheme;
    {
      ...
      if (scheme < length && *(ptr8 + scheme) == u'#') {
        scheme++;
        check = CheckCanonical(ptr8, scheme, length, u'￾');
        if ((check & Check::DisplayCanonical) == 0) {
...
Int32 Uri___::ParseSchemeCheckImplicitFile(Char* uriString, Int32 length, ParsingError& err, Flags& flags, UriParser& syntax) {
  ...
  Int32 j;
  for (j = i; j < length && *(uriString + j) != u':'; j++) {
  }
  ...
  Char c;
  if ((c = *(uriString + i + 1)) == u':' || c == u'|') {
    if (UriHelper::IsAsciiLetter(*(uriString + i))) {
      if ((c = *(uriString + i + 2)) == u'\\' || c == u'/') {
...
ParsingError Uri___::CheckSchemeSyntax(ReadOnlySpan<Char> span, UriParser& syntax) {
  ...
  switch (span.get_Length().get()) {
    ...
    case 4:
      ...
      break;
    case 5:
      if (1752462448 == (((UInt32)c2 << 24) | ((UInt32)ToLowerCaseAscii(span[1]) << 16) | ((UInt32)ToLowerCaseAscii(span[2]) << 8) | ToLowerCaseAscii(span[3])) && ToLowerCaseAscii(span[4]) == u's') {
        syntax = UriParser::in::HttpsUri;
        return ParsingError::None;
      }
      break;
    case 6:
      if (1835100524 == (((UInt32)c2 << 24) | ((UInt32)ToLowerCaseAscii(span[1]) << 16) | ((UInt32)ToLowerCaseAscii(span[2]) << 8) | ToLowerCaseAscii(span[3])) && ToLowerCaseAscii(span[4]) == u't' && ToLowerCaseAscii(span[5]) == u'o') {
  ...
  for (Int32 i = 1; i < span.get_Length(); i++) {
    Char c3 = span[i];
    if ((UInt32)(c3 - 97) > 25u && (UInt32)(c3 - 65) > 25u && (UInt32)(c3 - 48) > 9u && c3 != u'+' && c3 != u'-' && c3 != u'.') {
      return ParsingError::BadScheme;
    }
  }
...
Int32 Uri___::CheckAuthorityHelper(Char* pString, Int32 idx, Int32 length, ParsingError& err, Flags& flags, UriParser syntax, String& newHost) {
  ...
  Char c;
  if (idx == length || (c = *(pString + idx)) == u'/' || (c == u'\\' && StaticIsFile(syntax)) || c == u'#' || c == u'?') {
  ...
  if ((flags2 & UriSyntaxFlags::MayHaveUserInfo) != 0) {
    for (; j < i; j++) {
      if (j == i - 1 || *(pString + j) == u'?' || *(pString + j) == u'#' || *(pString + j) == u'\\' || *(pString + j) == u'/') {
        j = idx;
        break;
      }
      if (*(pString + j) != u'@') {
  ...
  if (c == u'[' && syntax->InFact(UriSyntaxFlags::AllowIPv6Host) && IPv6AddressHelper::IsValid(pString, j + 1, i)) {
  ...
  } else if (c <= u'9' && c >= u'0' && syntax->InFact(UriSyntaxFlags::AllowIPv4Host) && IPv4AddressHelper::IsValid(pString, j, i, false, StaticNotAny(flags, Flags::ImplicitFile), syntax->InFact(UriSyntaxFlags::V1_UnknownUri))) {
  ...
  if (i < length && *(pString + i) == u'\\' && (flags & Flags::HostTypeMask) != Flags::Zero && !StaticIsFile(syntax)) {
  ...
  if ((flags & Flags::HostTypeMask) == Flags::Zero) {
    ...
    if (syntax->InFact(UriSyntaxFlags::AllowAnyOtherHost)) {
      flags |= Flags::BasicHostType;
      for (i = idx; i < length && *(pString + i) != u'/' && *(pString + i) != u'?' && *(pString + i) != u'#'; i++) {
      }
      if (flag3) {
      ...
      Boolean flag4 = false;
      Int32 num5 = idx;
      for (i = idx; i < length && (!flag4 || (*(pString + i) != u'/' && *(pString + i) != u'?' && *(pString + i) != u'#')); i++) {
        if (i < idx + 2 && *(pString + i) == u'.') {
          flag4 = true;
          continue;
        }
        err = ParsingError::BadHostName;
...
Uri::in::Check Uri___::CheckCanonical(Char* str, Int32& idx, Int32 end, Char delim) {
  ...
  for (i = idx; i < end; i++) {
    ...
    if (c == delim || (delim == u'?' && c == u'#' && _syntax != nullptr && _syntax->InFact(UriSyntaxFlags::MayHaveFragment))) {
      break;
    }
    if (c == u'?') {
      if (get_IsImplicitFile() || (_syntax != nullptr && !_syntax->InFact(UriSyntaxFlags::MayHaveQuery) && delim != u'￾')) {
        check |= Check::ReservedFound;
        flag2 = true;
        flag = true;
      }
    ...
    if (c == u'/' || c == u'\\') {
      if ((check & Check::BackslashInPath) == 0 && c == u'\\') {
        check |= Check::BackslashInPath;
      }
      if ((check & Check::DotSlashAttn) == 0 && i + 1 != end && (*(str + i + 1) == u'/' || *(str + i + 1) == u'\\')) {
        check |= Check::DotSlashAttn;
      }
      continue;
    }
    if (c == u'.') {
      if (((check & Check::DotSlashAttn) == 0 && i + 1 == end) || *(str + i + 1) == u'.' || *(str + i + 1) == u'/' || *(str + i + 1) == u'\\' || *(str + i + 1) == u'?' || *(str + i + 1) == u'#') {
        check |= Check::DotSlashAttn;
      }
      continue;
    }
    if ((c > u'"' || c == u'!') && (c < u'[' || c > u'^')) {
      switch (c.get()) {
        ...
        default:
          ...
          if (i + 2 < end && (c = UriHelper::DecodeHexChars(*(str + i + 1), *(str + i + 2))) != u'￿') {
            if (c == u'.' || c == u'/' || c == u'\\') {
...
Array<Char> Uri___::GetCanonicalPath(Array<Char> dest, Int32& pos, UriFormat formatAs) {
  ...
  if (formatAs == UriFormat::UriEscaped) {
    if (InFact(Flags::ShouldBeCompressed)) {
      ...
      if (_syntax->InFact(UriSyntaxFlags::UnEscapeDotsAndSlashes) && InFact(Flags::PathNotCanonical) && !get_IsImplicitFile()) {
        {
          Char* pch = dest;
          UnescapeOnly(pch, pos, end, u'.', u'/', _syntax->InFact(UriSyntaxFlags::ConvertPathSlashes) ? u'\\' : u'￿');
        }
      }
    } else if (InFact(Flags::E_PathNotCanonical) && NotAny(Flags::UserEscaped)) {
      String text = _string;
      if (securedPathIndex != 0 && text[securedPathIndex + _info->Offset.Path - 1] == u'|') {
        text = text->Remove(securedPathIndex + _info->Offset.Path - 1, 1);
        text = text->Insert(securedPathIndex + _info->Offset.Path - 1, ":");
      }
      dest = UriHelper::EscapeString(MemoryExtensions::AsSpan(text, _info->Offset.Path, _info->Offset.Query - _info->Offset.Path), dest, end, !get_IsImplicitFile(), u'?', u'#');
    } else {
      _string->CopyTo(_info->Offset.Path, dest, end, _info->Offset.Query - _info->Offset.Path);
      end += _info->Offset.Query - _info->Offset.Path;
    }
    ...
    if (InFact(Flags::ShouldBeCompressed) && _syntax->InFact(UriSyntaxFlags::UnEscapeDotsAndSlashes) && InFact(Flags::PathNotCanonical) && !get_IsImplicitFile()) {
      {
        Char* pch2 = dest;
        UnescapeOnly(pch2, pos, end, u'.', u'/', _syntax->InFact(UriSyntaxFlags::ConvertPathSlashes) ? u'\\' : u'￿');
      }
    }
  }
  if (securedPathIndex != 0 && dest[securedPathIndex + pos - 1] == u'|') {
    dest[securedPathIndex + pos - 1] = u':';
  }
  if (InFact(Flags::ShouldBeCompressed)) {
    ...
    if (formatAs == UriFormat::UriEscaped && NotAny(Flags::UserEscaped) && InFact(Flags::E_PathNotCanonical)) {
      dest = UriHelper::EscapeString(rt::newstr<String>(dest, pos, end - pos), dest, pos, !get_IsImplicitFile(), u'?', u'#');
      end = pos;
    }
  ...
  if (formatAs != UriFormat::UriEscaped && InFact(Flags::PathNotCanonical)) {
    ...
    {
      Char* pStr = array;
      dest = UriHelper::UnescapeString(pStr, pos, end, dest, pos, u'?', u'#', u'￿', unescapeMode, _syntax, false);
    }
  } else {
    pos = end;
  }
...
Array<Char> Uri___::Compress(Array<Char> dest, Int32 start, Int32& destLength, UriParser syntax) {
  ...
  for (start = (UInt16)(start - 1); num5 != start; num5 = (UInt16)(num5 - 1)) {
    Char c = dest[num5];
    if (c == u'\\' && syntax->InFact(UriSyntaxFlags::ConvertPathSlashes)) {
      c = (dest[num5] = u'/');
    }
    if (c == u'/') {
      num = (UInt16)(num + 1);
    ...
    if (num3 != 0) {
      if ((!syntax->NotAny(UriSyntaxFlags::CanonicalizeAsFilePath) || (num3 <= 2 && c == u'/' && num5 != start)) && c == u'/' && (num2 == num5 + num3 + 1 || (num2 == 0 && num5 + num3 + 1 == destLength)) && num3 <= 2) {
...
String Uri___::CombineUri(Uri basePart, String relativePart, UriFormat uriFormat) {
  ...
  if (basePart->get_IsDosPath() && (c == u'/' || c == u'\\') && (relativePart->get_Length() == 1 || (relativePart[1] != u'/' && relativePart[1] != u'\\'))) {
    Int32 num = basePart->get_OriginalString()->IndexOf(u':');
    ...
    num = basePart->get_OriginalString()->IndexOf(u':', num + 1);
    return String::in::Concat(MemoryExtensions::AsSpan(basePart->get_OriginalString(), 0, num + 1), relativePart);
  }
  if (StaticIsFile(basePart->get_Syntax()) && (c == u'\\' || c == u'/')) {
    if (relativePart->get_Length() >= 2 && (relativePart[1] == u'\\' || relativePart[1] == u'/')) {
      if (!basePart->get_IsImplicitFile()) {
  ...
  if (c == u'/' || (c == u'\\' && flag)) {
    if (relativePart->get_Length() >= 2 && relativePart[1] == u'/') {
  ...
  c = (basePart->get_Syntax()->InFact(UriSyntaxFlags::MayHaveQuery) ? u'?' : u'￿');
  Char c2 = ((!basePart->get_IsImplicitFile() && basePart->get_Syntax()->InFact(UriSyntaxFlags::MayHaveFragment)) ? u'#' : u'￿');
...
String Uri___::Unescape(String path) {
  ...
  Int32 destPosition = 0;
  dest = UriHelper::UnescapeString(path, 0, path->get_Length(), dest, destPosition, u'￿', u'￿', u'￿', UnescapeMode::Unescape | UnescapeMode::UnescapeAll, nullptr, false);
  return rt::newstr<String>(dest, 0, destPosition);
}

String Uri___::EscapeString(String str) {
  if (str != nullptr) {
    return UriHelper::EscapeString(str, true, UriHelper::get_UnreservedReservedTable(), u'?', u'#');
  }
  return String::in::Empty;
}
...
Boolean Uri___::IsReservedCharacter(Char character) {
  if (character != u';' && character != u'/' && character != u':' && character != u'@' && character != u'&' && character != u'=' && character != u'+' && character != u'$') {
    return character == u',';
  }
  return true;
}
...
Boolean Uri___::IsExcludedCharacter(Char character) {
  if (character > u' ' && character < u'' && character != u'<' && character != u'>' && character != u'#' && character != u'%' && character != u'"' && character != u'{' && character != u'}' && character != u'|' && character != u'\\' && character != u'^' && character != u'[' && character != u']') {
    return character == u'`';
  }
  return true;
}
...
void Uri___::InitializeUri(ParsingError err, UriKind uriKind, UriFormatException& e) {
  if (err == ParsingError::None) {
    if (get_IsImplicitFile()) {
      if (NotAny(Flags::DosPath) && uriKind != UriKind::Absolute && (uriKind == UriKind::Relative || (_string->get_Length() >= 2 && (_string[0] != u'\\' || _string[1] != u'\\')))) {
        _syntax = nullptr;
        _flags &= Flags::UserEscaped;
        e = nullptr;
        return;
...
Boolean Uri___::InternalIsWellFormedOriginalString() {
  ...
  }
  {
    ...
    if (!get_IsAbsoluteUri()) {
      ...
      return (CheckCanonical(ptr2, idx, _string->get_Length(), u'￾') & (Check::EscapedCanonical | Check::BackslashInPath)) == Check::EscapedCanonical;
    }
    if (get_IsImplicitFile()) {
      return false;
    }
    ...
    if (InFact(Flags::AuthorityFound)) {
      ...
      if (idx >= _info->Offset.User || _string[idx - 1] == u'\\' || _string[idx] == u'\\') {
        return false;
      }
      if (InFact(Flags::DosPath | Flags::UncPath) && ++idx < _info->Offset.User && (_string[idx] == u'/' || _string[idx] == u'\\')) {
        return false;
      }
    }
    ...
    if (get_IsDosPath() && _string[_info->Offset.Path + get_SecuredPathIndex() - 1] == u'|') {
      return false;
    }
    if ((_flags & Flags::CanonicalDnsHost) == Flags::Zero && get_HostType() != Flags::IPv6HostType) {
      idx = _info->Offset.User;
      Check check = CheckCanonical(ptr2, idx, _info->Offset.Path, u'/');
    ...
    if ((_flags & (Flags::SchemeNotCanonical | Flags::AuthorityFound)) == (Flags::SchemeNotCanonical | Flags::AuthorityFound)) {
      ...
      if (idx + 1 >= _string->get_Length() || *(ptr2 + idx) != u'/' || *(ptr2 + idx + 1) != u'/') {
        return false;
      }
    }
  }
...
String Uri___::UnescapeDataString(String stringToUnescape) {
  ...
  Int32 num = stringToUnescape->IndexOf(u'%');
  if (num == -1) {
  ...
  UriHelper::UnescapeString(stringToUnescape, num, stringToUnescape->get_Length(), dest, u'￿', u'￿', u'￿', UnescapeMode::Unescape | UnescapeMode::UnescapeAll, nullptr, false);
...
Uri Uri___::ResolveHelper(Uri baseUri, Uri relativeUri, String& newUriString, Boolean& userEscaped) {
  ...
  if (text[0] == u'#' && !baseUri->get_IsImplicitFile() && baseUri->get_Syntax()->InFact(UriSyntaxFlags::MayHaveFragment)) {
    newUriString = baseUri->GetParts(UriComponents::HttpRequestUrl | UriComponents::UserInfo, UriFormat::UriEscaped) + text;
    return nullptr;
  }
  if (text[0] == u'?' && !baseUri->get_IsImplicitFile() && baseUri->get_Syntax()->InFact(UriSyntaxFlags::MayHaveQuery)) {
  ...
  if (text->get_Length() >= 3 && (text[1] == u':' || text[1] == u'|') && UriHelper::IsAsciiLetter(text[0]) && (text[2] == u'\\' || text[2] == u'/')) {
    ...
    case UriFormat::SafeUnescaped:
      {
        ...
        Int32 destPosition = 0;
        dest = UriHelper::UnescapeString(_string, 0, _string->get_Length(), dest, destPosition, u'￿', u'￿', u'￿', UnescapeMode::EscapeUnescape, nullptr, false);
        return rt::newstr<String>(dest, 0, destPosition);
      }default:
      rt::throw_exception<ArgumentOutOfRangeException>("format");
  }
...
void Uri___::CreateThisFromUri(Uri otherUri) {
  ...
  if (InFact(Flags::MinimalUriInfoSet)) {
    ...
    if (InFact(Flags::NotDefaultPort)) {
      while (otherUri->_string[num] != u':' && num > otherUri->_info->Offset.Host) {
        num--;
      }
      if (otherUri->_string[num] != u':') {
