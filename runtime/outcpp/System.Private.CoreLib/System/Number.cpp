...
void Number::NumberToStringFormat(ValueStringBuilder& sb, NumberBuffer& number, ReadOnlySpan<Char> format, NumberFormatInfo info) {
    ...
    num9 = num2;
    {
      ...
      while (num9 < format.get_Length() && (c = *(ptr + num9++)) != 0) {
        switch (c.get()) {
          ...
          case 'E':
          case 'e':
            if ((num9 < format.get_Length() && *(ptr + num9) == '0') || (num9 + 1 < format.get_Length() && (*(ptr + num9) == '+' || *(ptr + num9) == '-') && *(ptr + num9 + 1) == '0')) {
              while (++num9 < format.get_Length() && *(ptr + num9) == '0') {
              }
              flag = true;
            }
            continue;
  ...
  Boolean flag3 = false;
  {
    ...
    Char c;
    while (num9 < format.get_Length() && (c = *(ptr3 + num9++)) != 0 && c != ';') {
      if (num11 > 0 && (c == '#' || c == '.' || c == '0')) {
        while (num11 > 0) {
      ...
      switch (c.get()) {
        ...
        case '0':
          if (num11 < 0) {
            ...
            c = (Char)((*ptr2 != 0) ? ((Int32)(*(ptr2++))) : ((Int32)((num10 > num6) ? '0' : '\0')));
          }
          if (c != 0) {
            sb.Append(c);
            if (flag2 && num10 > 1 && num12 >= 0 && num10 == span[num12] + 1) {
        ...
        case 'e':
          {
            ...
            if (flag) {
              if (num9 < format.get_Length() && *(ptr3 + num9) == '0') {
                num18++;
              } else if (num9 + 1 < format.get_Length() && *(ptr3 + num9) == '+' && *(ptr3 + num9 + 1) == '0') {
                positiveSign = true;
              } else if (num9 + 1 >= format.get_Length() || *(ptr3 + num9) != '-' || *(ptr3 + num9 + 1) != '0') {
                sb.Append(c);
                break;
              ...
              while (++num9 < format.get_Length() && *(ptr3 + num9) == '0') {
                num18++;
              }
              if (num18 > 10) {
                num18 = 10;
  ...
  }
  {
    ...
    while (true) {
      ...
      if ((UInt32)c2 <= 34u) {
        ...
        if (--section == 0) {
          if (num >= format.get_Length() || *(ptr + num) == '\0' || *(ptr + num) == ';') {
            break;
          }
          return num;
        }
...
UInt16 Number::ExtractFractionAndBiasedExponent(Half value, Int32& exponent) {
  ...
  UInt16 num2 = (UInt16)(num & 1023);
  exponent = ((num >> 10) & 31);
  if (exponent != 0) {
    num2 = (UInt16)(num2 | 1024);
    exponent -= 25;
  } else {
    exponent = -24;
  }
