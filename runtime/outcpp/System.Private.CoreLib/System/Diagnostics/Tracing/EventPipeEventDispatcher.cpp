...
void EventPipeEventDispatcher___::StartDispatchTask() {
  if (m_dispatchTask == nullptr) {
    ...
    m_dispatchTask = Task<>::in::get_Factory()->StartNew({(EventPipeEventDispatcher)this, &DispatchEventsToEventListeners}, CancellationToken::get_None(), TaskCreationOptions::LongRunning, TaskScheduler::in::get_Default());
  }
}
