...
void CounterGroup___::OnTimer() {
  ...
  }
  {
    ...
    TimeSpan t = utcNow - _nextPollingTimeStamp;
    if (t > TimeSpan::Zero && _pollingIntervalInMilliseconds > 0) {
      _nextPollingTimeStamp += TimeSpan::FromMilliseconds((Double)_pollingIntervalInMilliseconds * Math::Ceiling(t.get_TotalMilliseconds() / (Double)_pollingIntervalInMilliseconds));
    }
  }
}
