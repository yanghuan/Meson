...
Boolean CalendarData___::NlsLoadCalendarDataFromSystem(String localeName, CalendarId calendarId) {
  ...
  flag &= CallGetCalendarInfoEx(localeName, calendarId, 48u | num, iTwoDigitYearMax);
  flag &= CallGetCalendarInfoEx(localeName, calendarId, 2u, sNativeName);
  flag &= CallGetCalendarInfoEx(localeName, calendarId, 56u | num, sMonthDay);
  flag &= CallEnumCalendarInfo(localeName, calendarId, 5u, 31u | num, saShortDates);
  flag &= CallEnumCalendarInfo(localeName, calendarId, 6u, 32u | num, saLongDates);
...
Int32 CalendarData___::NlsGetTwoDigitYearMax(CalendarId calendarId) {
  if (!GlobalizationMode::get_Invariant()) {
    Int32 data;
    if (!CallGetCalendarInfoEx((String)nullptr, calendarId, 48u, data)) {
      return -1;
    }
    return data;
  }
...
Boolean CalendarData___::CallGetCalendarInfoEx(String localeName, CalendarId calendar, UInt32 calType, Int32& data) {
  return Interop::Kernel32::GetCalendarInfoEx(localeName, (UInt32)calendar, IntPtr::Zero, calType | 536870912u, IntPtr::Zero, 0, data) != 0;
}
...
Boolean CalendarData___::CallEnumCalendarInfo(String localeName, CalendarId calendar, UInt32 calType, UInt32 lcType, Array<String>& data) {
  ...
  if (lcType != 0 && (lcType & 2147483648u) == 0 && GetUserDefaultLocaleName() == localeName) {
...
String CalendarData___::GetUserDefaultLocaleName() {
  ...
  if (localeInfoEx > 0) {
    return rt::newstr<String>(ptr, 0, localeInfoEx - 1);
  }
  return "";
}
