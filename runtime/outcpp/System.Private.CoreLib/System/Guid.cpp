...
Boolean Guid::TryParseExactB(ReadOnlySpan<Char> guidString, GuidResult& result) {
  if (guidString.get_Length() != 38 || guidString[0] != u'{' || guidString[37] != u'}') {
...
Boolean Guid::TryParseExactD(ReadOnlySpan<Char> guidString, GuidResult& result) {
  ...
  if (guidString[8] != u'-' || guidString[13] != u'-' || guidString[18] != u'-' || guidString[23] != u'-') {
...
Boolean Guid::TryParseExactX(ReadOnlySpan<Char> guidString, GuidResult& result) {
  ...
  if (guidString.get_Length() == 0 || guidString[0] != u'{') {
  ...
  Int32 num = 3;
  Int32 num2 = MemoryExtensions::IndexOf(guidString.Slice(num), u',');
  if (num2 <= 0) {
  ...
  num = num + num2 + 3;
  num2 = MemoryExtensions::IndexOf(guidString.Slice(num), u',');
  if (num2 <= 0) {
    result.SetFailure(false, "Format_GuidComma");
    return false;
  }
  ...
  if ((UInt32)guidString.get_Length() <= (UInt32)(num + num2 + 1) || guidString[num + num2 + 1] != u'{') {
    result.SetFailure(false, "Format_GuidBrace");
    return false;
  }
  num2++;
  ...
  if (num + num2 + 1 >= guidString.get_Length() || guidString[num + num2 + 1] != u'}') {
...
Boolean Guid::TryParseHex(ReadOnlySpan<Char> guidString, UInt32& result, Boolean& overflow) {
  if (guidString.get_Length() != 0) {
    ...
    if ((UInt32)guidString.get_Length() > 1u && guidString[0] == u'0' && (guidString[1] | 32) == 120) {
      guidString = guidString.Slice(2);
    }
  }
  Int32 i;
  for (i = 0; i < guidString.get_Length() && guidString[i] == u'0'; i++) {
  }
  Int32 num = 0;
  UInt32 num2 = 0u;
...
Boolean Guid::IsHexPrefix(ReadOnlySpan<Char> str, Int32 i) {
  if (i + 1 < str.get_Length() && str[i] == u'0') {
